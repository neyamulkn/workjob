...
class BannerController extends Controller
{
    ...
    {
        $data['permission'] = $this->checkPermission('banners');
        ...
        $banners = Banner::leftJoin('pages', 'banners.page_name', 'pages.id');
            ...
            if($request->pages && $request->pages != 'all'){
                $banners->where('pages.slug', $request->pages);
            }
            $perPage = 15;
            if($request->show){
                $perPage = $request->show;
            }
        $data['banners'] = $banners->orderBy('position', 'asc')->select('banners.*', 'pages.title as page_title')->paginate($perPage);

...
class WalletController extends Controller
{
    ...
    public function walletHistory(Request $request){
        ...
        $data['withdraw_configure'] = SiteSetting::where('type', 'user_withdraw_configure')->where('status', 1)->first();
        return view('users.wallet.wallet')->with($data);
    }

    public function withdrawRequest(Request $request){
        $withdrawalConfig = SiteSetting::where('type', 'user_withdraw_configure')->where('status', 1)->first();
